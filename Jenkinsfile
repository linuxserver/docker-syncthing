pipeline {
  ...
  environment {
    ...
    EXT_USER = 'syncthing'
    EXT_REPO = 'syncthing'
    BUILD_VERSION_ARG = 'SYNCTHING_RELEASE'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-syncthing'
    CONTAINER_NAME = 'syncthing'
    DOCKERHUB_IMAGE = 'linuxserver/syncthing'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/syncthing'
    PR_DOCKERHUB_IMAGE = 'lspipepr/syncthing'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/syncthing") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/syncthing",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-syncthing",\
                       "backendUrl":"https://ghcr.io/linuxserver/syncthing",\
                       "publicUrl":"https://lscr.io/linuxserver/syncthing"}' || :
              else
